...
export function colorsFromDynamicScheme(dynamicScheme: DynamicScheme, applyBrightnessSuffix: boolean = false): Record<string, number> {
  const colors: Record<string, number> = {}
  for (const [name, color] of Object.entries(MaterialDynamicColors)) {
    if (color instanceof DynamicColor) {
      const suffix = applyBrightnessSuffix ? dynamicScheme.isDark ? 'Dark' : 'Light' : ''
